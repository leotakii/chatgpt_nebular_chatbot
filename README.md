Versão modificada que integra chat showcase component com a API do ChatGPT:

http://localhost:4200/#/chat/chat-showcase.component


## Como Executar

[Aviso] realizar as instalações e atualizações pertinentes à execução destes comandos:
```
npm install //realizar este comando tanto no Cliente quanto no Servidor
npm install -g @angular/cli
curl -sL https://deb.nodesource.com/setup_14.x -o setup_14.sh
sudo sh ./setup_14.sh
sudo apt update
sudo apt install nodejs
node -v //precisa ser o node.js 14
```

Cliente:
``` cd /nebular
sudo ng serve --port 4200
``` 

Servidor:
``` cd /nebular/src/backend 
node app.js //por padrao abre a porta 4400
```
...
O texto abaixo se refere ao template do Nebular:
